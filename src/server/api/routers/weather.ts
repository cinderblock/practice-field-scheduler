...
import { getWeatherForecast, getWeatherForDate } from "~/server/weatherService";
...
export const weatherRouter = createTRPCRouter({
	...
	getWeatherForDate: publicProcedure
...
function getMostCommonCode(codes: number[]): number {
	...
	return parseInt(Object.entries(counts).reduce((a, b) => {
		const aCount = counts[parseInt(a[0])] || 0;
		const bCount = counts[parseInt(b[0])] || 0;
